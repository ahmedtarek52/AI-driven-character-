...
namespace HuggingFace.API
{
    ...
    public class HuggingFaceAPI : MonoBehaviour
    {
        ...
        {
            string[] commandKeywords = { "use ", "cast ", "target ", "attack " };
            string lowered = input.ToLower().Trim();
            return commandKeywords.Any(keyword => lowered.StartsWith(keyword));
        }
        ...
        private async Task ProcessInput(string userInput)
        {
            try
            {
                if (IsCommand(userInput))
                {
                    ...
                    if (commandResponse == "target enemy 1" || 
                        commandResponse == "target enemy 2" || 
                        commandResponse == "use fireball" || 
                        commandResponse == "use shield")
                    {
                        _responseText.text = commandResponse;
                        if (CombatManager.instance != null)
                        {
                            CombatManager.instance.ProcessCommand("target enemy 1");
                        }
                    }
                    else
                    {
